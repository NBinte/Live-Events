...
import styles from "@/styles/new-live-events.module.css";
...
const mockEvents: SportsDBEvent[] = [
  {
    idEvent: "mock-1",
    strEvent: "Manchester United vs Bournemouth",
    ...
    idHomeTeam: "133612",
    idAwayTeam: "133614",
    dateEvent: "2025-12-16",
    strTime: "22:00:00",
    strStatus: "Not Started",
    strVenue: "Old Trafford",
    strCity: "Manchester",
    strCountry: "England",
    strTVStation: "Sky Sports, NBC Sports",
    intHomeScore: null,
    intAwayScore: null,
  ...
  {
    idEvent: "mock-2",
    strEvent: "Fenerbahçe vs Panathinaikos",
    strLeague: "EuroLeague",
    strSport: "Basketball",
    strHomeTeam: "Fenerbahçe",
    strAwayTeam: "Panathinaikos",
    idHomeTeam: "mock-home-2",
    idAwayTeam: "mock-away-2",
    dateEvent: "2025-12-16",
    strTime: "23:45:00",
    strStatus: "LIVE",
    strVenue: "Ülker Sports Arena",
    strCity: "Istanbul",
    strCountry: "Turkey",
    strTVStation: "Eurosport, DAZN",
    intHomeScore: "71",
    intAwayScore: "68",
    strHomeTeamBadge: null,
    strAwayTeamBadge: null,
...
export function LiveEvents({
  ...
  const fetchLiveEvents = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        setIsOnline(true);
        console.log(
          `Loaded ${data.events.length} events from ${data.source || "API"}`
        );
      } else {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch live events"
      );
      setIsOnline(false);

      if (events.length === 0) {
        // mock fallback (keeps page usable even if APIs are down)
        setEvents(mockEvents);
        setLastUpdated(new Date());
      }
        ...
        try {
          ...
          if (fallbackResponse.ok) {
            ...
            if (fallbackData.success) {
              ...
              setLastUpdated(new Date());
              console.log(
                `Loaded ${fallbackData.events.length} fallback events`
              );
            }
          }
        } catch (fallbackErr) {
  ...
  const isEventLive = (event: SportsDBEvent): boolean => {
    ...
    return liveStatuses.some((status) =>
      event.strStatus?.toUpperCase().includes(status)
    );
  };
  ...
  const TVChannelsDropdown = ({ tvStation }: { tvStation: string | null }) => {
    ...
    const channels =
      tvStation &&
      tvStation !== "TV info not available" &&
      tvStation.trim() !== ""
        ? tvStation
            .split(",")
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100 cursor-pointer transition-all duration-300 border-2 border-purple-200 hover:border-purple-300">
              <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-2">
                  <Image
                    src={
                      TV_CHANNEL_LOGOS[primaryChannel] ||
                      "/placeholder.svg?height=24&width=60&query=TV channel logo"
                    }
                    alt={primaryChannel}
                    width={30}
                    height={18}
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500">
                      {CHANNEL_REGIONS[channel] || "🌍 Global"}
                    </div>
                  </div>
  ...
  const VenueInfo = ({ event }: { event: SportsDBEvent }) => {
    ...
    const location =
      [event.strCity, event.strCountry].filter(Boolean).join(", ") ||
      "Location TBA";
  ...
  const LayoutSettingsPanel = () => {
    return (
      <div className={styles.liveEventsPage}>
            ...
            <Card className="bg-white shadow-lg border-0">
              <CardHeader className="text-center pb-6">
                ...
                <p className="text-gray-600">
                  Configure your display preferences using your account
                  settings.
                </p>
              </CardHeader>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setLayoutSettings((prev) => ({
                        ...prev,
                        maxEvents: Number.parseInt(e.target.value) || 6,
                      }))
                    }
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setLayoutSettings((prev) => ({
                            ...prev,
                            refreshInterval: Number.parseInt(value),
                          }))
                        }
                      >
    ...
    return (
      <Card className="hover:shadow-2xl transition-all duration-300 group border-2 border-gray-200 hover:border-blue-300 bg-white overflow-hidden">
        <CardContent className="p-0">
          <div
            className={`flex ${layoutSettings.compactMode ? "h-64" : "h-80"}`}
          >
            ...
            <div className="w-80 relative bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 flex items-center justify-center">
              ...
              {isLive && (
                <div className="absolute top-4 right-4 z-20">
                  <Badge className="bg-red-500 text-white animate-pulse border-0 text-sm px-3 py-1">
                    🔴 LIVE
                  </Badge>
                </div>
              )}
              ...
              <div className="relative z-10 flex items-center justify-between w-full px-6">
                ...
                <div className="text-center">
                  <div className="text-white text-2xl font-bold mb-2 bg-black/30 px-3 py-1 rounded-lg">
                    VS
                  </div>
                  <div
                    className={`w-6 h-6 rounded-full ${sportColor} mx-auto animate-pulse shadow-lg`}
                  ></div>
                </div>
            ...
            <div className="flex-1 flex flex-col">
              ...
              {(layoutSettings.showTVChannels ||
                ...
                <div
                  className={`${
                    layoutSettings.compactMode ? "h-16" : "h-24"
                      ...
                      <div className="h-full bg-gradient-to-br from-blue-100 to-indigo-100 rounded-lg border-2 border-blue-300 p-2">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <Badge className="bg-blue-200 text-blue-800 text-xs px-1 py-0">
                            {event.strTVStation
                              ? event.strTVStation.split(",").length
                              : 0}
                          </Badge>
                        </div>
              ...
              <div className="flex-1 p-4 bg-white">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <h3
                      className={`${
                        layoutSettings.compactMode ? "text-lg" : "text-xl"
          ...
          <div className="space-y-4">
            ...
            >
              {[...Array(skeletonCount)].map((_, i) => (
                <Card
                  key={i}
                  className="animate-pulse border-2 border-gray-100"
                >
                  {layout === "single" ? (
                    <CardContent className="p-0">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 ${
                    isLoading ? "animate-spin text-blue-600" : "text-purple-600"
                  }`}
                />
          ...
          {events.length === 0 ? (
            <Card className="border-2 border-dashed border-gray-300">
              <CardContent className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-4">
                  There are no live sports events at the moment.
                </p>
                <Button
                  onClick={fetchLiveEvents}
                  variant="outline"
            ...
            >
              {events.slice(0, maxEvents).map((event) => {
                if (layout === "single") {
                  return (
                    <HorizontalEventCard key={event.idEvent} event={event} />
                  );
                }
                    ...
                    <CardHeader className="pb-3 bg-gradient-to-br from-blue-50 to-purple-50">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col items-end space-y-1">
                          ...
                          <div
                            className={`rounded-full ${sportColor} shadow-lg animate-pulse w-4 h-4`}
                          ></div>
                        </div>
                      </div>
                    </CardHeader>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <div>
                              <p className="text-sm font-medium text-gray-700">
                                {event.strVenue || "Venue: N/A"}
                              </p>
                              <p className="text-xs text-gray-500">
                                {event.strCity || "City: N/A"},{" "}
                                {event.strCountry || "Country: N/A"}
                              </p>
                            </div>
                          </div>
                        </div>
